...
jQuery(document).ready(function ($) {
	 ...
	 *-------------------------------------------------*/
	 ...
	 jQuery(document).ready(function () {
	 	...
	 	var paction = location.search.split('action=')[1]
	 	if (paction == "reagendar"){

	 		var el = document.getElementsByClassName('make-appointment-btn');
	...
	jQuery(document).on('click','.save-time-slots',function(e){
		...
		//cambio horario
		var servicio	  = jQuery('body').find('input[name=servicioval]').val();
		console.log("servicio2 en save time click:"+servicio);
		...
		var dataString = 'day='+day+'&slot_title='+slot_title+'&start_time='+start_time+'&end_time='+end_time+'&meeting_time='+meeting_time+'&padding_time='+padding_time+'&servicio='+servicio+'&action=docdirect_add_time_slots';
	...
	jQuery(document).on('click','.delete-current-slot',function(e){
		...
		var servicio	  = jQuery('body').find('input[name=servicioval]').val();
		console.log("servicio2 en delete current click:"+servicio);
		...
		var dataString = 'day='+day+'&time='+time+'&servicio='+servicio+'&action=docdirect_delete_time_slot';
	...
	jQuery(document).on('click','.save-custom-time-slots',function(e){
		...
		//alert(complete_fields);

		jQuery('body').append(loader_fullwidth_html);
		//alert(start_time+' - '+end_time+' - '+meeting_time+' - '+padding_time);
		if( start_time == ''	||	end_time == ''	||	meeting_time == ''	||	padding_time == ''){
			//alert('vacios');
			jQuery('body').find('.docdirect-site-wrap').remove();
			jQuery.sticky(complete_fields, {classList: 'important', speed: 200, autoclose: 5000});
	...
	jQuery(document).on('click','.bk-step-next',function(e){
		...
		console.log("agendar cita en:"+bk_category);
		//Check step 1
		if( bk_category
			&&
			bk_service
			...
			//cambio horario
			var dataString = 'data_id='+data_id+'&servicio='+bk_category+'&action=docdirect_get_booking_step_two';
			jQuery.ajax({
				type: "POST",
				url: scripts_vars.ajaxurl,
				data: dataString,
	...
	jQuery(document).on('click','.get-detail',function(){
		...
		jQuery(this).parents('tr').find('.tg-appointmentdetail').slideToggle(200);
	});
	...
	jQuery(document).on('click','.get-process',function(){
		...
		if( type == 'approve' ) {
			var _title	 ="Aprobar";
			var _message	= "¿Estás seguro de aprobar la cita?";
		} if( type == 'cancel' ) {
				if( type2 == 'reagendar' ) {
					var _title	= "Reagendar cita"
					var _message	= "¿Estás seguro de reagendar?";
				}else{
					var _title	= "Cancelar cita"
					var _message	= "¿Estás seguro de Cancelar?";
				}
		}
				...
				'Yes': {
					...
					'action': function () {
						...
						jQuery.ajax({
							...
							success: function(response) {
								jQuery('body').find('.docdirect-loader-wrap').remove();
								console.log ("response:"+response.action_type)
								if( response.action_type == 'approved' ){
									...
									_this.parents('td').html(approved);
										...
										if( type2 == 'reagendar' ) {
											...
											window.location.href =  _this.data('url')+"&action=reagendar";
...
function docdirect_booking_calender(val){
	...
	var loder_html	= '<div class="docdirect-loader-wrap"><div class="docdirect-loader"><div class="bounce1"></div><div class="bounce2"></div><div class="bounce3"></div></div></div>';
	  ...
	  onChangeDateTime:function(dp,$input){
		...
		var _this	= jQuery(this);
		var data_id	= jQuery('.tg-appointmenttabcontent').data('id');
		var bk_service	= jQuery(".tg-appointmenttabcontent").find('.bk_category option:selected').val();


		jQuery('.booking-model-contents').append(loder_html);
		//cambio horario
		console.log("from calendar servicio:"+ bk_service);
		var dataString = 'slot_date='+slot_date+'&data_id='+data_id+'&servicio='+bk_service+'&action=docdirect_get_booking_step_two';
		jQuery.ajax({
			type: "POST",
			url: scripts_vars.ajaxurl,
			data: dataString,
