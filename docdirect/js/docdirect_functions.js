...
jQuery(document).ready(function ($) {
	...
	jQuery(window).load(function() {
		...
		jQuery.ajax({
			...
			data: jQuery('.do-forgot-form').serialize() + '&action=docdirect_get_encuesta',
			dataType: "json",
			success: function (response) {
				//console.log(response);
				jQuery('body').find('.docdirect-site-wrap').remove();
				if (response.type == 'success') {
					//console.log("sesion: "+getCookie("encuesta"));
					var encuestac = getCookie("encuesta");
  				if (encuestac != "") {
						//jQuery.sticky("1", {classList: 'success', speed: 200, autoclose: 5000, position: 'top-right', });
					}else{
						setCookie("encuesta", 1, 1);
						jQuery('#encuesta_id').val(response.bkcode+" "+response.bk_booking_date+" "+response.bk_slottime);
						jQuery('#encuesta_user_to').val(response.user_to);
						jQuery('#encuesta_doc').html("Cita: "+response.bkcode+"<br>"+response.doc_name+"<br>"+response.bk_booking_date+" "+response.bk_slottime);
			...
			success: function(response) {
				...
				if (response.type == 'success') {
					...
					/*if( response.html == 'refresh' ){
						...
						jQuery('.modal-encuesta').modal('hide');
	...
	*/
	...
	/* ---------------------------------------
     rtegistration Ajax LINK
     --------------------------------------- */
	jQuery('.do-registration-form-link').on('click', '.do-register-button', function (event) {
		event.preventDefault();
		var _this	= jQuery(this);
		jQuery('body').append(loder_html);

		var _authenticationform	= _this.parents('form.do-registration-form-link').serialize();
		...
		jQuery.ajax({
			...
			success: function (response) {
				...
				console.log(response);
				if (response.type === 'success') {
					//cambio push register
					if(typeof Android !== "undefined" && Android !== null) {
									 //Android.showToast("Bienvenido1.");
									 Android.setAlias(response.id)
				 } else {
						 alert("Not viewing in webview1");
				 }

					jQuery.sticky(response.message, {classList: 'success', speed: 200, autoclose: 5000,position: 'top-right',});
					if( response.profile_url ){
						jQuery('body').append(loder_html);
						//cambio para que se quede en la misma despues de crar usuario
	...
	/* ---------------------------------------
		 Login Ajax LINK
		 --------------------------------------- */
	jQuery('.do-login-form-link').on('click', '.do-login-button', function (event) {
		event.preventDefault();
		var _this	= jQuery(this);
		jQuery('body').append(loder_html);
		var _serialize	= _this.parents('form.do-login-form-link').serialize();
		...
		jQuery.ajax({
			...
			success: function (response) {
				...
				if (response.type === 'success') {
					...
					jQuery.sticky(response.message, {classList: 'success', speed: 200, autoclose: 5000,position: 'top-right',});
					if( response.url ){
						//cambio para que despues de login se quede en la misma ventana pero con sesión
						/*jQuery('body').append(loder_html);
	...
	jQuery(document).on('click', '.subscribe_me', function (event) {
		...
		var _this = jQuery(this);
		jQuery('body').append(loder_html);
		//alert(jQuery('.doc-formnewsletter').serialize());
		jQuery.ajax({
			type: 'POST',
			url: scripts_vars.ajaxurl,
