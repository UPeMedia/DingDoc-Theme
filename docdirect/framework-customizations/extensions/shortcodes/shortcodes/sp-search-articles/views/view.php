...
?>
<div class="sp-sc-search-articles tg-haslayout search-<?php echo esc_attr( $flag );?>">
			...
			<fieldset>
				<div class="doc-fieldsetholder search-input-wrap">
					<div class="form-group">
					   <input class="suggestarticles autocomplete-input" type="text" name="by_title" placeholder="<?php esc_html_e('Search articles by keyword','docdirect');?>" class="form-control">
					</div>							
				</div>
			</fieldset>
		</div>
	...
	<?php 
		$script	= "jQuery(document).ready(function (e) {
			jQuery( '.search-".esc_attr( $flag )." .suggestarticles' ).autocomplete({
				source: function( request, response ) {
					jQuery.ajax({
						type: 'POST',
						url: scripts_vars.ajaxurl,
						data: 'keyword=' + request.term + '&action=docdirect_search_articles',
						dataType: 'json',
