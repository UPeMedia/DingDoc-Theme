...
if ( ! function_exists( 'docdir_delete_avatar' ) ) {
	function docdir_delete_avatar() {
		...
		$get_avatar_id			= get_user_meta($user_identity, 'userprofile_media', true);
		$update_avatar 			= update_user_meta($user_identity, 'userprofile_media', '');
		if($update_avatar){

			wp_delete_attachment( $get_avatar_id, true ); //delete from media
...
if ( ! function_exists( 'docdir_delete_user_banner' ) ) {
	function docdir_delete_user_banner() {
		...
		/*-----------------------------Demo Restriction END--------------------------------*/
		$get_banner_id			= get_user_meta($user_identity, 'userprofile_banner', true);
		$update_avatar 			= update_user_meta($user_identity, 'userprofile_banner', '');

		if($update_avatar){

			wp_delete_attachment( $get_banner_id, true ); //delete from media
...
if ( ! function_exists( 'docdir_delete_email_logo' ) ) {
	function docdir_delete_email_logo() {
		...
		$get_media 		= get_user_meta($user_identity, 'email_media', true);
		$update_avatar  = update_user_meta($user_identity, 'email_media', '');

		if($update_avatar){

			wp_delete_attachment( $get_media, true ); //delete from media
...
if ( ! function_exists( 'docdir_delete_gallery_image' ) ) {
	function docdir_delete_gallery_image() {
		...
		if( isset( $image_id ) && isset( $gallery[$image_id] ) ){
			...
			wp_delete_attachment( $image_id, true ); //delete from media
		}else {
			$json['type']		=  'error';
			$json['message']		= esc_html__('Some error occur, please try again later.','docdirect');
		}
 ...
 * cambio stars
 */
if ( ! function_exists( 'docdirect_get_encuesta' ) ) {
	function docdirect_get_encuesta() {
		global $current_user, $wp_roles,$userdata,$post;
		$flag = 0;
		//devuelve los ultimos 7 dias
		function get_last_days($date)
		{
		    $cur_date = strtotime($date);
		    //$pre_monday  =$cur_date -(6*24*60*60);
				$pre_monday  =$cur_date -(10*24*60*60);
		...
		if( apply_filters('docdirect_is_user_logged_in','check_user') === false ){
			...
			$hoy = date("Y-m-d");
			$now = strtotime("now");
			$now = strtotime("-6 hour", $now);
			$horanow = date('H',$now);

			$fechasrangos = get_last_days($hoy);
			$dia1 = date('Y-m-d',$fechasrangos['start']);//"2020-03-01";
			//$dia2 = date('Y-m-d',$fechasrangos['end']);//"2020-03-07";
			...
			//de 7dias antes - ahorita
			$meta_query_args[] = array(
									array(
											'key' => 'bk_timestamp',
											'value' => array(strtotime($dia1), $now ),
											'compare' => 'BETWEEN',
											'type' => 'NUMERIC'
									),
							);
			...
			if( $query->have_posts() ):
				 while($query->have_posts()) : $query->the_post();
				 	global $post;
						...
						if (($horanow> $hora) && ($bk_status == "approved")){
							$bk_code2 = get_post_meta($post->ID, 'bk_code',true);
							$bk_user_to2 = get_post_meta($post->ID, 'bk_user_to',true);
							$doc_name2 = docdirect_get_username($bk_user_to2);
							$bk_booking_date2  = get_post_meta($post->ID, 'bk_booking_date',true);
							$bk_slottime2 	  = get_post_meta($post->ID, 'bk_slottime',true);
							$bk_status2 = get_post_meta($post->ID, 'bk_status',true);
							$flag = 1;
						}
			 ...
			 if ($flag == 1){
					$json['type']	= 'success';
					$json['message']	= "Cita: ".$bk_code2."<br>Doctor:".$doc_name2;
					$json['bkcode']	= $bk_code2;
					$json['bk_booking_date']	= $bk_booking_date2;
					$json['bk_slottime']	= $bk_slottime2;
					$json['dia1']	= strtotime($dia1);
					$json['dia2']	= date('Y-m-d H:i',$now);
					$json['user_to'] = $bk_user_to2;
					$json['doc_name'] = $doc_name2;
					$json['status'] = $bk_status2;
					echo json_encode($json);
					die;
				}else{
					$json['type']	= 'error';
					$json['message']	= "no hay encuesta pendiente";
					echo json_encode($json);
					die;
				}
		}
	}
	add_action('wp_ajax_docdirect_get_encuesta','docdirect_get_encuesta');
	add_action( 'wp_ajax_nopriv_docdirect_get_encuesta', 'docdirect_get_encuesta' );
}
...
if ( ! function_exists( 'docdirect_make_review' ) ) {
	function docdirect_make_review() {
		...
		if(  !empty( $_POST['user_rating'] )
			&& !empty( $_POST['user_rating2'] )
			&& !empty( $_POST['user_rating3'] )
			&& !empty( $_POST['user_to'] )
		) {

			$user_subject	   = sanitize_text_field($_POST['user_subject']);
			$user_rating	   = sanitize_text_field( $_POST['user_rating'] );
			$user_rating2	   = sanitize_text_field( $_POST['user_rating2'] );
			$user_rating3	   = sanitize_text_field( $_POST['user_rating3'] );
			$promedio = round(($user_rating + $user_rating2 + $user_rating3)/3,1);
			$user_description  = "Servicio del médico:".$user_rating."\nServicio del personal de clínica:".$user_rating2."\nServicio de Dingdoc:".$user_rating3;
			...
			if( isset( $dir_review_status ) && $dir_review_status == 'publish' ) {
				...
				$json['message']	= "Gracias por tu calificación.";
				$json['html']	   = '';
			}
			...
			$json['type']		= 'error';
			$json['message']	 = "Ingresa todos los campos.-";
			echo json_encode($json);
			die;
		}
...
if (!function_exists('docdirect_locateme_snipt')) {
	...
	?>
    	<div class="locate-me-wrap">
            ...
            <input type="text"  autocomplete="on" id="location-address" value="<?php echo esc_attr( $location );?>" name="geo_location" placeholder="Ubicación" class="form-control">
...
if (!function_exists('docdirect_profile_settings')) {
	function  docdirect_profile_settings(){
		//cambio ajustes para secciones de ajustes de perfil
		$list	= array(
			'media'	 			=> 'media',
			'gallery'			=> 'gallery',
			'basics'	 		=> 'basics',
 ...
 * @return {}
 */
 /*cambio funcion orden secciones en perfil */
 ...
 * @return
 */
if (!function_exists('docdirect_is_chat_enabled')) {
    function docdirect_is_chat_enabled() {
        $is_chat = '';
		if (function_exists('fw_get_db_settings_option')) {
			$is_chat = fw_get_db_settings_option('private_chat');
		}

		return !empty( $is_chat ) ? $is_chat : '';
    }
}
 ...
 * @return array
 */
if (!function_exists('docdirect_get_distance_scale')) {
	function docdirect_get_distance_scale() {
		if (function_exists('fw_get_db_settings_option')) {
			$dir_distance_type = fw_get_db_settings_option('dir_distance_type');
		} else {
