...
if ( ! function_exists( 'docdirect_add_time_slots' ) ) {
	function docdirect_add_time_slots(){
		...
		//cambio horario
		$servicio = esc_attr( $_POST['servicio'] );

		//echo 'servicio en function add_time_slots'.$servicio;

		if ($padding_time == '' || $padding_time == '0'){
				$padding_time = 0;
		}
		if( empty( $day )
		   || empty( $start_time )
		...
		//cambio horarios
		// graba horarios
		update_user_meta( $user_identity, 'default_slots_'.$servicio, $default_slots );


		$json['slots_data']	   = docdirect_get_default_slots($day,$servicio,'return');
		$json['message_type']	 = 'success';
...
if ( ! function_exists( 'docdirect_save_custom_slots' ) ) {
	function docdirect_save_custom_slots(){
		global $current_user, $wp_roles,$userdata,$post;
		$user_identity		 = $current_user->ID;
		$timeslots_object	 = ($_POST['custom_timeslots_object']);
