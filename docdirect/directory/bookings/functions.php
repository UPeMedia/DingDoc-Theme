...
if ( ! function_exists( 'docdirect_get_default_slots' ) ) {
	function docdirect_get_default_slots($day='',$servicio,$return_type="return"){
		...
		$default_slots	= array();
		$default_slots = get_user_meta($user_identity , 'default_slots_'.$servicio , true);
		//cambio horarios
		//echo "<pre>";
					...
					<div class="tg-box">
						...
						<?php if( !empty( $default_slots[$day.'-details'][$key]['slot_title'] ) ){?>
                        	<!-- <span class="tg-title"><?php //echo esc_attr( $default_slots[$day.'-details'][$key]['slot_title'] );?></span> -->
													<span class="tg-title">Espacio</span>
                        <?php }?>
...
if ( ! function_exists( 'docdirect_get_booking_step_two' ) ) {
	function docdirect_get_booking_step_two(){
		global $current_user, $wp_roles,$userdata,$post;
		do_action('docdirect_is_action_allow');
		...
		$default_slots = array();
		//cambio horario
		$default_slots = get_user_meta($user_id , 'default_slots_'.$servicio, false);
		$time_format   = get_option('time_format');
		//print_r($default_slots);
        ...
        foreach( $todays_defaults as $key => $value ){
			...
			//is expired
			if( $is_expired === 'yes' || $slot_status === 'disabled' ){
				$slotClass		= 'tg-booked';
                $slot_status	= 'disabled';
			} else{
				$slotClass		= 'tg-available';
            ...
            <div class="form-group tg-pay-radiobox">
              <label>
                ...
                <span><?php esc_html_e('I will pay locally.','docdirect');?></span>
						...
						<div class="form-group tg-pay-radiobox">
              <label>
                <input type="radio" value="seguro" name="payment">
                <span>Seguro Médico</span></label>
            </div>
            <?php
	...
	function docdirect_get_booking_step_five($user_identity){
        ...
        <div class="bk-thanks-message">
            <div class="tg-message">
              <h2>Gracias</h2>
              <div class="tg-description">
                <p>Cita agendada.<?php //echo //force_balance_tags( $thank_you );?></p>
              </div>
            </div>
        </div>
		<?php
