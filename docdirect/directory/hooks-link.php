...
if ( ! function_exists( 'docdirect_get_linked_term_options' ) ) {
	function docdirect_get_linked_term_options($current='',$taxonomyName='locations',$sub_category_terms) {
		//This gets top layer terms only.  This is done by setting parent to 0.
...
if (!function_exists('docdirect_do_check_user_type')) {
	...
	add_filter( 'docdirect_do_check_user_type', 'docdirect_do_check_user_type', 10, 1 );
}
 ...
 * @return
 */
if (!function_exists('docdirect_get_user_type')) {
	function docdirect_get_user_type($user_identity){
	...
	add_filter( 'docdirect_get_user_type', 'docdirect_get_user_type', 10, 1 );
}
...
if ( ! function_exists( 'docdirect_get_users_base_slug' ) ) {
	function docdirect_get_users_base_slug(){
		$slug = 'author';
		if (function_exists('fw_get_db_settings_option')) {
			$slug = fw_get_db_settings_option('user_page_slug', $default_value = 'author');
		}
 ...
 * @return array
 */
if (!function_exists('docdirect_search_filters_link')) {
	add_action( 'docdirect_search_filters_link', 'docdirect_search_filters_link' );
	function docdirect_search_filters_link() {
	global $wp_query;
	...
	<div class="search-filters-wrap">
       <div class="doc-widget doc-widgetsearch">
          <div class="doc-widgetheading">
            <h1 class="titulomodal">Buscar</h1>					
          </div>

					<input type="hidden" class="form-control" value="<?php echo esc_attr( $page_idn );?>" name="page_id" >
					<div class="doc-sortby" style="display:none;">
                     <span class="doc-select">
                      <select name="sort_by" class="sort_by" id="sort_by">
                          ...
                          <option value="distance" selected="">Por distancia</option>
              ...
              <fieldset>
                <?php if( isset( $dir_keywords ) && $dir_keywords === 'enable' ){?>
                  <div class="form-group">
                    <input type="text" class="form-control" value="<?php echo esc_attr( $by_name );?>" name="by_name" placeholder="Buscar">
                  </div>
                <?php }?>
                <div class="form-group">
                  ...
                  <script type="text/template" id="tmpl-load-subcategories">
                    <div class="doc-widget doc-widgetfilterspecialist" style="display: none">
                      <div class="doc-widgetheading">
...
if (!function_exists('docdirect_is_demo_site')) {
	function docdirect_is_demo_site($message=''){
		...
		if( isset( $_SERVER["SERVER_NAME"] )
			&& $_SERVER["SERVER_NAME"] === 'dingdocth.com' ){
			$json['type']	   =  "error";
 ...
 * @return
 */
if (!function_exists('docdirect_search_articles')) {

    function docdirect_search_articles() {
        global $wpdb;
        $json = array();
		$suggestions = array();
		...
		if( $cust_query->have_posts() ){
			while( $cust_query->have_posts() ) {
				...
				$suggestion['url'] 		= get_the_permalink();
    ...
    add_action('wp_ajax_docdirect_search_articles', 'docdirect_search_articles');
    add_action('wp_ajax_nopriv_docdirect_search_articles', 'docdirect_search_articles');
}
 ...
 * @return
 */
if (!function_exists('docdirect_search_questions')) {

    function docdirect_search_questions() {
        global $wpdb;
        $json = array();
		$suggestions = array();
		$s	 = esc_attr($_POST['keyword']);
    ...
    add_action('wp_ajax_docdirect_search_questions', 'docdirect_search_questions');
    add_action('wp_ajax_nopriv_docdirect_search_questions', 'docdirect_search_questions');
}
 ...
 * @return
 */
if (!function_exists('docdirect_is_action_allow')) {
	function docdirect_is_action_allow(){
		global $current_user;
		$user_identity = $current_user->ID;
		$json	= array();

		$activation_status = get_user_meta($user_identity, 'profile_status', true);
		$verify_user 	   = get_user_meta($user_identity, 'verify_user', true);

		if( isset( $activation_status ) && ( $activation_status === 'de-active' || $activation_status === '' )  ){
			$json['type'] 	 	 	 = 'error';
			$json['message'] 	 	 = esc_html__('Please activate your account before perform this action.', 'docdirect');

			echo json_encode($json);
			die;
		} elseif( isset( $verify_user ) && ( $verify_user === 'off' || $verify_user === '' )  ){
			$json['type'] 	 	 	 = 'error';
			$json['message'] 	 	 = esc_html__('You are not a verified user, please contact to site administrator to get verify, then you will be able do this action.', 'docdirect');
 ...
 * @return {}
 */
if (!function_exists('docdirect_file_uploader')) {

    function docdirect_file_uploader() {
        global $current_user, $wp_roles, $userdata, $post;
        $user_identity = $current_user->ID;
        ...
        if (isset($uploaded_item['file'])) {
            ...
            $profile_meta = get_user_meta($user_identity, $type, true);
			$data_array = array();
			if (!empty($profile_meta['file_data'])) {
				...
				update_user_meta($user_identity, $type, $profile_meta);
			} else {
				$data_array = array(
					'file_type' => $type,
				...
				update_user_meta($user_identity, $type, $data_array);
			}
            ...
            $ajax_response = array(
                ...
                'file_icon'  => $file_icon,
                'file_title' => $file_title,
				'file_url'   => $uploaded_item['url']
            );

            echo json_encode($ajax_response);
            die;
        } else {
            $ajax_response = array('success' => false, 'reason' => esc_html__('Brochure upload failed!','docdirect'));
            echo json_encode($ajax_response);
            die;
        }
    }

    add_action('wp_ajax_docdirect_file_uploader', 'docdirect_file_uploader');
    add_action('wp_ajax_nopriv_docdirect_file_uploader', 'docdirect_file_uploader');
}
 ...
 * @return String
 */
if (!function_exists('docdirect_delete_profile_file')) {

    function docdirect_delete_profile_file() {
        global $current_user, $wp_roles, $userdata, $post;
        $user_identity = $current_user->ID;
        $json = array();
        ...
        $profile_brochure = get_user_meta($user_identity, $file_type, true);
		...
		$update_brochure = update_user_meta($user_identity, $file_type, $profile_brochure);

		wp_delete_attachment($attach_id); //delete photos from media
		if ($update_brochure) {
			$json['type'] = 'success';
			$json['message'] = esc_html__('File deleted.', 'docdirect');
		} else {
			$json['type'] = 'error';
			$json['message'] = esc_html__('An error occurred, please try again later', 'docdirect');
		}
        echo json_encode($json);
        exit;
    }

    add_action('wp_ajax_docdirect_delete_profile_file', 'docdirect_delete_profile_file');
    add_action('wp_ajax_nopriv_docdirect_delete_profile_file', 'docdirect_delete_profile_file');
}
