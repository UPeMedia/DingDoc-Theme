...
if (!function_exists('docdirect_scripts')) {
    ...
    //se paso a functions.php
		...
		wp_enqueue_style('docdirect_responsive_style', get_template_directory_uri() . '/css/responsive.css', array('chat_module_css'), $theme_version->get( 'Version' ));
		...
		wp_enqueue_script('docdirect_functions', get_template_directory_uri() . '/js/docdirect_functions.js', array('jquery','jquery-ui-core','jquery-ui-sortable','plupload','jquery-ui-slider','jquery-ui-autocomplete','wp-util'), $theme_version->get( 'Version' ), true);
    ...
    //cambio stars - ahora en todas las paginas
    ...
    wp_enqueue_script('countdown', get_template_directory_uri() . '/js/countdown.js', array(), $theme_version->get( 'Version' ), false);
		...
		if ( ( is_page_template('directory/user-profile.php') && isset($_GET['ref']) == 'private-chat' ) || is_author()) {



			if( ( is_page_template('directory/user-profile.php') && isset($_GET['ref']) == 'private-chat' ) ){
        ...
        if ( ( is_page_template('directory/user-profile-link.php') && isset($_GET['ref']) == 'private-chat' ) || is_author()) {



    			if( ( is_page_template('directory/user-profile-link.php') && isset($_GET['ref']) == 'private-chat' ) ){
    ...
    //Conditional Scripts
		...
		if ( is_page_template( 'directory/user-profile-link.php' ) ) {
...
if (!function_exists('docdirect_admin_scripts')) {
    ...
    function docdirect_admin_scripts() {
		...
		$calendar_locale	  = empty( $calendar_locale ) ? 'en' : $calendar_locale;
		...
		wp_localize_script('docdirect_admin_functions', 'localize_vars', array(
			...
			'select_options' => esc_html__('Select Option','docdirect'),

			'no_results_text' => esc_html__('No result match','docdirect'),
...
if (!function_exists('docdirect_prepare_social_sharing')) {
    ...
    function docdirect_prepare_social_sharing($default_icon = 'false' , $title = 'Share' , $title_enable = 'true' , $classes = '',$thumbnail='') {
        ...
        if (isset($social_twitter['gadget'])
            ...
            $output .= '<li class="tg-twitter"><a class="tg-social-twitter" target="_blank" href="https://twitter.com/intent/tweet?text=' . htmlspecialchars(urlencode(html_entity_decode(get_the_title(), ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8') . '&url=' . urlencode( esc_url( get_permalink() ) ) . '&via=' . urlencode( !empty( $twitter_username )? $twitter_username : get_bloginfo( 'name' ) ) . '"  ><i class="fa fa-twitter"></i><span>' . $twitter . '</span></a></li>';
...
if (!function_exists('docdirect_prepare_profile_social_sharing')) {
    ...
    function docdirect_prepare_profile_social_sharing($thumbnail='',$user_id,$description='') {
		...
		if ( ( isset($social_facebook) && $social_facebook == 'enable' )
			...
			$output .= "<h3 style='text-align:center;'>Compartir Tarjeta Virtual</h3>";
        ...
        $output .= '<a class="tg-social-facebook" href="http://www.facebook.com/share.php?u=' . urlencode( esc_url( $user_url) ) . '&title='.urlencode($username).'&description='.urlencode($description).'&picture='.urlencode($thumbnail).'" onclick="window.open(this.href, \'post-share\',\'left=50,top=50,width=600,height=350,toolbar=0\'); return false;"><span><i class="fa fa-facebook fa-2x"></i></span></a>';
        //$output .= '<a class="tg-social-facebook" href="#" ><span><i class="fa fa-share-alt fa-2x" aria-hidden="true"></i></span></a>';
          //$output .= '<a href="whatsapp://send?text=Tarjeta virtual de '.urlencode($username).' en ' . urlencode( esc_url( $user_url) ) . '" data-action="share/whatsapp/share">Share via Whatsapp</a>';
          //echo "tipo:". $_GET['tipo'];
          if ($_GET['tipo'] == "link"){
            $output .= '<a href="whatsapp://send?text=Tarjeta virtual de '.urlencode($username).' en ' . urlencode( esc_url( $user_url) ) . '" data-action="share/whatsapp/share"><span><i class="fa fa-whatsapp fa-2x"></i></span></a>';
          }else{
            $output .= '<a href="https://api.whatsapp.com/send?text=Perfil de '.urlencode($username).' en ' . urlencode( esc_url( $user_url) ) . '" target="_blank"><span><i class="fa fa-whatsapp fa-2x"></i></span></a>';
          }
      }
        ...
        //cambio
				/*$output .= '<li class="tg-twitter"><a class="tg-social-twitter" target="_blank" href="https://twitter.com/intent/tweet?text=' . htmlspecialchars(urlencode(html_entity_decode($username, ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8') . '&url=' . urlencode( esc_url( $user_url ) ) . '&via=' . urlencode( !empty( $twitter_username )? $twitter_username : get_bloginfo( 'name' ) ) . '"  ><i class="fa fa-twitter"></i><span>' . $twitter . '</span></a></li>';
...
if(!function_exists('docdirect_get_users')){
	...
	function docdirect_get_users(){
		...
		$users['']	= esc_html__('Select Organizer','docdirect');
