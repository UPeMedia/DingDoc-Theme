...
if (!function_exists('docdirect_mailchimp_list')) {
    function docdirect_mailchimp_list() {
		$mailchimp_list[]='';
		$mailchimp_list[0] = esc_html__('Select List','docdirect');
		$mailchimp_option	= '';
...
if ( ! function_exists( 'docdirect_user_social_mehthods' ) ) {
	...
	function docdirect_user_social_mehthods( $userfields ) {
		...
		$userfields['user_address']		= "Dirección";
		$userfields['zip']				= "Zip";
    if ( ! empty( $user->roles ) && ( is_array( $user->roles ) && in_array( 'professional', $user->roles ) ) || $user->roles === 'professional' ) {
		//$userfields['tagline']			= "Colegiado";

    $userfields['colegiado']			= "Colegiado";
    }else{
      $userfields['certificado']			= "Certificado";
      $userfields['polizacarnetseguro']			= "Póliza/Carnet de seguro";
    }
