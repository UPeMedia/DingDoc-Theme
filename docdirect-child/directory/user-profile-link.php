        ...
        <div class="col-lg-9 col-md-9 col-sm-8 col-xs-12 pull-right">
			<?php
				if( isset( $_GET['ref'] )
					...
					get_template_part('directory/templates/user','medical-documents');
				} *//*else if( isset( $_GET['ref'] )
					 && $_GET['ref'] === 'insurance'
					 && $url_identity == $user_identity
					 && apply_filters('docdirect_do_check_user_type',$url_identity ) === true
				 ){
					get_template_part('directory/templates/user','insurance-documents');
				}*/ else if( isset( $_GET['ref'] )
					&& $_GET['ref'] === 'settings'
					&& $url_identity == $user_identity
				){
					...
					if( apply_filters('docdirect_do_check_user_type',$url_identity ) === true  ){
						if( apply_filters('docdirect_is_setting_enabled',$url_identity,'appointments' ) === true  ){
							get_template_part('directory/bookings/templates/user','bookings-link');
						}
					} else{
						get_template_part('directory/bookings/templates/customer','bookings-link');
					}
					...
					if( apply_filters('docdirect_do_check_user_type',$url_identity ) === true  ){
						if( apply_filters('docdirect_is_setting_enabled',$url_identity,'appointments' ) === true  ){
							get_template_part('directory/bookings/templates/user','bookingsm-link');
						}
					} else{
						get_template_part('directory/bookings/templates/customer','bookingsm-link');
					}
					...
					if( apply_filters('docdirect_do_check_user_type',$user_identity ) === true){
						if( apply_filters('docdirect_is_setting_enabled',$user_identity,'favorite' ) === true ){
							get_template_part('directory/templates/user','favourites-link');
						}
					} else{
						get_template_part('directory/templates/user','favourites-link');
					}
					...
					&& $_GET['ref'] === 'private-chat'
					&& $url_identity == $user_identity
					&& !empty($is_chat) && $is_chat === 'yes'
				){
					get_template_part('directory/templates/dashboard','messaging');
				} else if( isset( $_GET['ref'] )
					&& $_GET['ref'] === 'favourites'
					&& $url_identity == $user_identity
				){
...
<script type="text/template" id="tmpl-load-gallery">
	<figure class="data-gallery-wrap">
		<a href="javascript:;"><img height="56" width="56" src="{{data.url}}"></a>
		<div class="tg-img-hover" data-attachment_id="{{data.attachment_id}}">
			<a href="javascript:;" class="del-gallery" data-attachment="{{data.attachment_id}}"><i class="fa fa-plus"></i><i class="fa fa-refresh fa-spin"></i></a>
		</div>
				...
				<td class="award-data edit-me-row"colspan="2">
					<div class="tg-education-form tg-haslayout">
						<div class="award-data">
							...
							<div class="col-md-8 col-sm-6 col-xs-12">
								<div class="form-group">
									<input class="form-control award_datepicker" value="{{element.date}}" name="awards[{{index}}][date]" type="text" placeholder="<?php esc_attr_e('Award Date','docdirect');?>">
								</div>
							</div>
							<div class="col-md-12 col-sm-12 col-xs-12">
								<div class="form-group">
